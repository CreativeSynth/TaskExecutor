...
for file in target:
    ...
    reader = csv.reader(open(os.path.relpath(file, current_position), "rt", encoding='utf8', newline=''))
    next(reader)
    ...
    # Set run function
    try:
        run = globals()['run_' + methods[task_name]]
    except:
    ...
    result_path = os.path.relpath(reference_path, current_position)
